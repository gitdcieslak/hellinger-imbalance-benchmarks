...
1. We provide empirical evidence that, under severe class imbalance, conventional ranking and reliability summaries can underdescribe threshold-mediated minority accessibility, including near-neighbor ranking cases (for example, Bagged HDDT vs LightGBM) with materially different accessibility geometry.
2. We use reachability trajectories as an analytical lens for minority accessibility under threshold policy movement, emphasizing trajectory structure rather than single-threshold endpoints.
3. We show this non-equivalence signal across multiple evidence layers: model-family contrasts, fixed-architecture neural perturbation transitions, and calibration-accessibility interaction.
4. We provide operational descriptors (persistence, smoothness, jump localization) as practical summarization tools for threshold-policy controllability, while treating morphology vocabulary as provisional empirical shorthand.
