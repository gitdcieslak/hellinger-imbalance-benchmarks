...
As a compact anchor for the manuscript's main claim, Bagged HDDT and LightGBM have near-neighbor ranking quality (`AUROC 0.8389` vs `0.8479`; `AP 0.3434` vs `0.3462`) but materially different accessibility geometry (`Recovery 0.9024` vs `0.5485`; `Max Recall Jump 0.6100` vs `0.2218`; `Operational Smoothness 0.1741` vs `0.2624`; `Threshold-Survival Persistence 0.5397` vs `0.4413`) [source: `paper/manuscript_tables/table_allocator_family_summary_v2.md`]. This pair illustrates the central non-equivalence signal: similar ranking summaries do not imply similar threshold-policy behavior.
