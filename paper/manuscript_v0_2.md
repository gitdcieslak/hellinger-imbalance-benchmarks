...
Where our focus differs is operational emphasis under severe imbalance: minority accessibility persistence rather than aggregate acceptance coverage, elasticity localization (where threshold sensitivity concentrates), and threshold-policy robustness in constrained deployment settings. We therefore position reachability as an operational reinterpretation and elevation of class-conditional threshold trajectories for severe imbalance analysis.

### 2.4 Calibration and Decision-Focused Probability Estimation

Calibration literature has established post-hoc methods and reliability diagnostics such as Platt scaling, isotonic regression, ECE, and Brier score [CITATION: Platt scaling], [CITATION: isotonic calibration], [CITATION: calibration foundations], [CITATION: modern calibration analysis]. We rely on these standard tools and calibration remains valuable for probability reliability.

Our contribution in this area is not a new calibration method. Instead, we examine how calibration interacts with operational accessibility trajectories under severe imbalance. Current evidence suggests that reliability improvements (lower ECE/Brier) can coexist with less smooth threshold-accessibility behavior in the same setting.
...
This section addresses a key reviewer concern directly: “isn’t reachability just recall?” Recall at a fixed threshold is one point on `R(t)`. Reachability analysis studies trajectory behavior across thresholds, including:
...
As acknowledged in Related Work, this object partially overlaps with class-conditional coverage and selective prediction trajectory analysis. Our emphasis differs in targeting minority accessibility persistence, elasticity localization, and operational threshold robustness under severe imbalance.
...
In baseline analyses, CART often aligns with quantized behavior (minimal threshold response), XGBoost is consistently cliff-like in current evidence, LightGBM appears conservative in the cited runs, and HDDT/bagged HDDT often align with broader accessibility patterns. These labels are empirical shorthand over multi-metric behavior.
...
Reviewer-facing caveat: morphology labels are heuristic and threshold-grid dependent. We therefore treat them as recurring empirical patterns, not formal classes.
...
- `mlp_bce`: `AUROC=0.7068`, `AP=0.2410`, `recall@0.50=0.1346`, `recall@0.01=0.7867`, `operational_smoothness=0.4178`, `max_recall_jump=0.4061`, regime `cliff_allocator`.
- `mlp_oversampled`: `AUROC=0.8351`, `AP=0.2989`, `recall@0.50=0.4973`, `recall@0.01=0.7041`, `operational_smoothness=0.5391`, `max_recall_jump=0.0678`, regime `smooth_allocator`.
- `mlp_weighted`: `AUROC=0.7732`, `AP=0.2658`, `recall@0.50=0.5789`, `recall@0.01=0.8506`, `operational_smoothness=0.6080`, `max_recall_jump=0.1783`, regime `smooth_allocator`.

These shifts appear consistent with optimization/imbalance-pressure sensitivity and support the claim that operational accessibility is not architecture-only.
...
Potential objection: “does oversampling merely shift thresholds?” Interval-derivative evidence suggests more than pure shift: jump localization and elasticity concentration structure change materially.
...
Operationally, this framing is relevant where threshold policy acts as a control mechanism: queue-managed review systems, human-in-the-loop triage, and other settings where small threshold moves can create large access changes.
...
3. **Terminology and overlap risk:** reachability and related terms partially overlap with selective prediction / class-conditional coverage traditions.
4. **Provisional morphology labels:** recurring labels are heuristic empirical shorthand, not formal ontology.
...
8. **No formal theory claims:** the manuscript does not provide a complete theoretical characterization or causal proof.
...
This manuscript argues that under severe class imbalance, threshold-mediated operational accessibility should be analyzed as a first-class empirical object. Current evidence suggests that static ranking and calibration summaries can obscure trajectory behavior that is operationally consequential. Reachability trajectories, elasticity localization, and accessibility persistence provide a practical lens for this analysis. Within a fixed MLP architecture, objective/sampling perturbations can move behavior from cliff-like to smoother patterns, supporting the view that accessibility morphology is sensitive to imbalance-pressure mechanisms. Calibration findings further suggest a meaningful tension between reliability improvements and operational smoothness.

We present these results as an empirical + conceptual framing contribution. We treat morphology language as provisional, preserve explicit non-claims, and frame this draft as a foundation for deeper, targeted follow-up rather than a complete theory or benchmark endpoint.
