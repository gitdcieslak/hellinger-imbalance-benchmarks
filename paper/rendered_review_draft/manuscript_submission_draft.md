...
Severe class imbalance has long been studied through cost-sensitive learning, sampling/reweighting strategies, and imbalance-aware tree criteria. Foundational work includes cost-sensitive classification formulations [@Elkan2001], synthetic minority over-sampling [@Chawla2002], and broad imbalance surveys [@HeGarcia2009], [@Krawczyk2016]. Class-weighting and reweighting are now standard components in both classical and neural pipelines [@Buda2018], [@JohnsonKhoshgoftaar2019].

For Hellinger-based trees, the original HDDT line [@CieslakChawla2008] and later robustness/skew-insensitivity analysis [@CieslakEtAl2012] motivate inclusion of HDDT-style families as strong rare-event baselines in our analysis.

HDDT was introduced to reduce class-skew sensitivity of conventional split criteria by using Hellinger distance at split selection, with explicit motivation in rare-event settings and strong empirical performance under imbalance [@CieslakEtAl2012]. The current manuscript does not claim that prior HDDT work studied accessibility geometry. Instead, we revisit HDDT-family behavior through an operational accessibility lens to evaluate threshold-mediated minority access and policy controllability.
...
Imbalance evaluation is commonly anchored by ROC/AUROC and PR/AP analyses [@DavisGoadrich2006], [@SaitoRehmsmeier2015], often supplemented by threshold-specific confusion-matrix metrics. Operating-point analysis is well established in decision-oriented classification and cost-curve traditions [@Fawcett2006], [@DrummondHolte2006]. These metrics remain valuable and are used throughout.
...
Our closest conceptual neighbors are reject-option classification, selective prediction, and risk-coverage analysis [@Chow1970], [@ElYanivWiener2010], [@GeifmanElYaniv2017]. These traditions analyze confidence-thresholded action sets and risk/coverage behavior.
...
Calibration literature has established post-hoc methods and reliability diagnostics such as Platt scaling, isotonic regression, ECE, and Brier score [@Platt1999], [@ZadroznyElkan2002], [@NiculescuMizilCaruana2005], [@Guo2017]. We rely on these tools, and calibration remains valuable for probability reliability.
...
Decision-theoretic classification and utility-sensitive thresholding emphasize threshold choice under asymmetric costs [@Elkan2001], [@Fawcett2006]. Operational ML traditions emphasize deployment constraints, system maintenance/monitoring risk, and human-in-the-loop workflow design in triage/review systems [@Sculley2015], [@Breck2017], [@Amershi2019]. Our perspective is complementary: instead of optimizing only a single operating point, we focus on accessibility evolution as thresholds move under workload and risk constraints.
...
![Operational accessibility lens.](../manuscript_figures/figure_0_operational_accessibility_lens.png){width=95%}
...
![Mean reachability transition.](../manuscript_figures/figure_1_reachability_mean_transition.png){width=80%}



To evaluate whether the aggregate pattern is stable or dataset-specific, we then inspect per-dataset reachability panels.

![Dataset-level reachability panels.](../manuscript_figures/figure_2_reachability_by_dataset.png){width=95%}
...
![Accessibility regime map: max jump vs threshold-survival persistence.](../manuscript_figures/figure_accessibility_regime_map_variant_a_v2.png){width=80%}
...
![Elasticity interval concentration heatmap.](../manuscript_figures/figure_3_elasticity_interval_heatmap.png){width=80%}
...
![Support vs persistence scatter.](../manuscript_figures/figure_4_support_vs_persistence.png){width=80%}
...
![Calibration geometry deltas.](../manuscript_figures/figure_5_calibration_geometry_deltas.png){width=80%}
